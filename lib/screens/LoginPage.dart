  ...
  _login() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        bool isDetail = userDetails.exists && ((userDetails.data() as Map<String, dynamic>)['is_detail'] ?? false);
        log('TTTTTT: $userDetails.data()');
        log('VVVVVV: $isDetail');
        if (isDetail) {
          // 사용자의 디테일 정보가 있는 경우
          Get.offAll(() => HomePage(initialPageIndex: 1));
        } else {
          // 사용자의 디테일 정보가 없는 경우
          Get.offAll(() => HomePage(initialPageIndex: 0));
        }
